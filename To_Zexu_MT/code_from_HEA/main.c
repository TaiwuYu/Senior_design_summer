...
#pragma acc data copy(c_r,eta_r,mu_r,hc_r,g_v,g_mod2,mu_cr)
#pragma acc data copy(c_k,eta_k,mu_k,hc_k,mu_ck)
{
...
{	GetEintC(timestep);
    ...
    #pragma acc update host(c_r,eta_r,c_k,eta_k,hc_r,hc_k,mu_r,mu_cr,Sig)
	OutputMed(timestep);
	 	Fchem=cal_Fchem();
		Felas=cal_Felas();

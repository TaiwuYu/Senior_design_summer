...
//#define Cijklc (6700)
#define Cijklc (10000)
#define MISFIT_STRAIN (0.007) // Misfit strain
...
#pragma acc data present(vEps,E_oo,hc_r,eta_r)
#pragma acc parallel loop collapse(3)
	x_loop{ 
	V6_loop{
            vEps[pIDX][mi] = hc_r[pxyz]*E_oo[mi];       // Initial guess of virtual strain field
...
#pragma acc data present(Tau_0,vEps,E_oo,hc_r,eta_r)
#pragma acc parallel loop collapse(3)
    x_loop{
        V6_loop{
            Tau_0[pIDX][mi] -= vEps[pIDX][mi] - hc_r[pxyz]*E_oo[mi];
...
#pragma acc data present(Tau,Sij0,Cij0,Cij2,hc_r,EpsAvg,E_oo,eta_r)
#pragma acc parallel loop collapse(3)
	    x_loop{
	        V6_loop{
	            Tau[pIDX][mi] = 0.;
...
#pragma acc data present(Tau,Sij0,Cij0,Cij2,hc_r,E_oo,eta_r)
...
{
double sum=0;
#pragma acc data present(vEps,hc_r,E_oo,Sij0,Sig)
#pragma acc parallel loop collapse(3) reduction(+:sum)
x_loop{
	V6_loop{sum+=Sig[pIDX][mi]*(vEps[pIDX][mi]-hc_r[pxyz]*E_oo[mi]);

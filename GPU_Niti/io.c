...
int i;
		...
		sprintf(filename,"%s/cr_515_t%d.vtk",outdir,timestep);
		write_vtk(&c_r[0][0][0],filename,"cr");
#pragma omp parallel for private(filename)
for(i=0;i<V;i++)
	{	sprintf(filename,"%s/etar_515_%d_t%d.vtk",outdir,i,timestep);
		write_vtk(&eta_r[i][0][0][0],filename,"etar");
}
...
//		sprintf(filename,"%s/hetar3_t%d.vtk",outdir,timestep);
